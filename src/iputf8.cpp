...
// these two functions work together to sanitize a string that has been processed
// by RapidJson. Don't use it on strings that still contains need-to-escape characters sucha as \b, \n, \
// we did experiment and confirmed that RapidJson doesn't sanitized code above 127
// but does that for 0x01 to 0x1F, and escape everything that needs escape
// so don't worry, there won't be anything needs escape
...
const char* ValueTypeString[] = {"NULL", "STRING", "BOOL", "INT", "UINT", "INT64", "UINT64", "DOUBLE"};
...
IpVer getCidrIpFromUtf8Ip(const std::string& utf8Ip, std::string& res) {
  ...
  while (i < utf8Ip.size()) {
    ...
    if (val == 0xC3) { //e.g., rapidJson will read \u00FF  as 0xC3 0xBF
      ...
      ipV6[ipIndex] += 0x40; // the diff between UTF8 code and the original code, e.g., 0xBF->0xFF, 0xBE->0xFE
      i += 2;
    } else if (val == 0xC2) {
      ipV6[ipIndex] = (unsigned char)utf8Ip[i + 1];
      i += 2;
  ...
  if (areFirstTenBytesZero && ipV6[10] == 0xFF && ipV6[11] == 0xFF) { // ipV4
    retVal = IPVER_V4;
    res = to_string((long long unsigned int)ipV6[12]) + '.' + to_string((long long unsigned int)ipV6[13]) + '.'
          + to_string((long long unsigned int)ipV6[14]) + '.' + to_string((long long unsigned int)ipV6[15]);
  } else {
    char hexRep[8];
    retVal = IPVER_V6;
...
//tool to get JSON encode IP from human readable IP. For IPv6 input, we don't accept abbreviated like ::12:3:4:5, and only accept a-f, not A-F
string getJsonIpFromCidrIp(const string& cidrIp) {
  ...
  if (cidrIp.find(':') == string::npos) { // IPv4
    ...
    while (i < n) {
      while (i < n && cidrIp[i] != '.') {
        bytes[index] *= 10;   //convert number string to int. Move onto next digit
...
void sanitizeNonAsciiInput(const string& input, string& output) {
  ...
  if (!needSanitize) {
    ...
    output.clear();
    output.reserve(inputSize + 300); //according to our experience, there won't be more than 60 non-ascii code
